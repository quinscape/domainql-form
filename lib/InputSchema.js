...
function executeValidationPlan(values, validationPlan) {
    if (validationPlan[0] === PLAN_SCALAR_LIST) {
        var result = validationPlan[1](values);
    ...
    for (var i = 0; i < validationPlan.length; i += 2) {
            ...
            if (fnOrArray[0] === PLAN_COMPLEX_LIST) {
                if (fieldValue) {
                    ...
                    for (var j = 0; j < fieldValue.length; j++) {
                        var err = executeValidationPlan(fieldValue[j], fnOrArray.slice(1));
                        if (err) {
                ...
                if (fieldValue) {
                    var _err = executeValidationPlan(fieldValue, fnOrArray.slice(1));
                    if (_err) {
                        errors = errors || {};
                        errors[name] = _err;
                    }
...
var InputSchema = function () {
    ...
    _createClass(InputSchema, [{
        ...
        key: "fromValues",
        value: function fromValues(typeName, value) {
            var baseTypeDef = this.getType(typeName);
            if (!isInputType(baseTypeDef)) {
                throw new Error(typeName + " is not a known input type: " + JSON.stringify(baseTypeDef));
            }
        ...
        value: function validate(type, values) {
            ...
            var errors = executeValidationPlan(values, validationPlan);
