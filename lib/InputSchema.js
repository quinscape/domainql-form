...
function getValidationPlan(inputSchema, typeName) {
    ...
    for (var i = 0; i < inputFields.length; i++) {
        ...
        if (isScalar) {
            ...
            var elementType = actualType.ofType;
...
function convertValue(inputSchema, typeRef, value, toScalar) {
    ...
    if (isScalarType(typeRef)) {
        ...
        if (inputSchema.debug) {
            console.log(value, "( type", typeRef, ") ==", toScalar ? "toScalar" : "fromScalar", "=> ", result, typeof result === "undefined" ? "undefined" : _typeof(result), path);
        }
