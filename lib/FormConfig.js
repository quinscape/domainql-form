...
var setFormValueAction = (0, _mobx.action)("Set Form-Value", function (root, name, value) {
    (0, _lodash4.default)(root, name, value);
});
...
var FormConfig = function () {
    ...
    function FormConfig(opts) {
        ...
        this.handleBlur = function (fieldContext, value) {
            try {
    ...
    _createClass(FormConfig, [{
        ...
        key: "handleChange",
        value: function handleChange(fieldContext, value) {
            try {
                ...
                var isScalar = unwrapped.kind === "SCALAR";
                var error = isScalar ? _InputSchema2.default.validate(unwrapped.name, value) : null;
                ...
                if (error) {
                    ...
                    if (validation && validation.validateField) {
                        var highLevelResult = validation.validateField(fieldContext, value);

                        if (highLevelResult) {
                            if (Array.isArray(highLevelResult)) {
                                errorsForField = [value].concat(_toConsumableArray(highLevelResult));
                            } else {
                    ...
                    if (!errorsForField) {
                        converted = isScalar ? _InputSchema2.default.valueToScalar(unwrapped.name, value) : value;
                    }
                }
                ...
                var index = findError(currentErrors, qualifiedName);
                if (index < 0) {
                    if (errorsForField) {
                        // ADD ERRORS
